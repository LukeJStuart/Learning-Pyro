...
def find_stockmarkets():
    # You could cardcode the stockmarket names for nasdaq and newyork, but it
    # is more flexible if we just look for every available stockmarket.
    markets = []
    with Pyro4.locateNS() as ns:
        for market, market_uri in \
         ns.list(prefix='example.stockmarket.').items():
            print('found market', market)
            markets.append(Pyro4.Proxy(market_uri))
    if not markets:
        raise ValueError('no markets found! (have you started the ' +
                         'stock markets first?)')
    return markets
